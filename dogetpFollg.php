...
$rstUsrProfile = mysql_query($strSql);
	...
	$strSql = "SELECT * FROM cat_users WHERE intId IN (SELECT intFriend FROM tbl_usr_friendship WHERE intUser = " . $vId . ")";
						...
						<div>
					        ...
					        if($blnActions){
					        	if($objUsrs['intId']!=$uId){
									...
									?>
										<input onclick="DoFollow('btnFollow_<?php echo $uId; ?>_<?php echo $objUsrs['intId']; ?>_2', 36,<?php echo $uId; ?>, <?php echo $objUsrs['intId']; ?>, '<?php echo $arrLang['btnFollow']; ?>', '<?php echo $arrLang['btnUnFollow']; ?>');" type="button" id="btnFollow_<?php echo $uId; ?>_<?php echo $objUsrs['intId']; ?>_2" value="<?php echo $arrLang['btnFollow']; ?>" class="dark_36px" style=" display:inline-block; position:relative; margin:0px 0px 0px 0px; ">
									<?php
									}else{
									?>
										<input onclick="DoFollow('btnFollow_<?php echo $uId; ?>_<?php echo $objUsrs['intId']; ?>_2', 36,<?php echo $uId; ?>, <?php echo $objUsrs['intId']; ?>, '<?php echo $arrLang['btnFollow']; ?>', '<?php echo $arrLang['btnUnFollow']; ?>');" type="button" id="btnFollow_<?php echo $uId; ?>_<?php echo $objUsrs['intId']; ?>_2" value="<?php echo $arrLang['btnUnFollow']; ?>" class="clear_36px" style=" display:inline-block; position:relative; margin:0px 0px 0px 0px; ">
									<?php
									};
									unset($rstToFollow);
					        	};
